...
pub async fn run_dot_server(
    ...
    let listener = TcpListener::bind(dot_addr)
        .await
        .expect("Failed to bind DoT listener");
            ...
            if let Ok(mut tls_stream) = acceptor.accept(stream).await {
                ...
                if let Ok(size) = tls_stream.read(&mut buf).await {
                    ...
                    if let Ok(message) = Message::from_vec(query_bytes) {
                        ...
                        if let Some((response, expiry)) =
                            cache.get(&domain).map(|v| v.value().clone())
                        {
                        ...
                        let upstream_socket = TokioUdpSocket::bind("0.0.0.0:0")
                            .await
